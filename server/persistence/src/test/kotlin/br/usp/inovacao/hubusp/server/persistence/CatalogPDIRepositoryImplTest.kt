...
class CatalogPDIRepositoryImplTest {
    ...
    @Test
    fun `it filters by all field`() {
        // given
        val category = "Centro de Pesquisa em Engenharia"
        val campus = "Butantã"
        val params = PDISearchParams(categories = setOf(category), campus = campus)
        ...
        // then
        assertTrue { result.isNotEmpty() }
        assertTrue { result.all { it.category == category && it.campus == campus } }
    }
