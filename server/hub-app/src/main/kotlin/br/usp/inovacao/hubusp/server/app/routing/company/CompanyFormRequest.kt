...
class CompanyFormRequest {
    ...
    constructor(json: String, logoFile: File?) {
        this.json = json
        this.form = (Json { explicitNulls = false }).decodeFromString<CompanyForm>(this.json)
        this.logo =
            if (logoFile != null) {
                Image(logoFile)
            } else {
    ...
    companion object {
        suspend fun recv(call: ApplicationCall): CompanyFormRequest {
    ...
    fun createCsvTable() = listOf(this.form.toCsvRow())
}
